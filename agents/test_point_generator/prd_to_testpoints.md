...
- `e2e`：值是一个数组（跨端端到端集成测试，如"客户端下单 → 后端处理 → 后台查看"这类贯穿多端的完整业务流程。**仅当测试点天然横跨两个及以上端时归入此组**，单一端可验证的测试点不得放入 `e2e`）
...
- 测试点横跨两个及以上端（如"客户端下单 → 后端处理 → 后台查看"完整链路）→ 放入 `e2e`。仅在单一端可验证的测试点**不得**放入 `e2e`，应归入对应单一端。
...
"e2e": \[
{"id": "08", "detail": "客户端发帖 → 后端审核入库 → 后台处理 → 客户端状态同步", "priority": "P0", "source": "prd", "type": "normal"}
]
}
